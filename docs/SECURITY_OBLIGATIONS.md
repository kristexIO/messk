...
| Accessible modal and call controls | Implemented and regression-tested | `messk/src/components/AccessibleModalFrame.tsx` gives critical modals labelled dialog semantics, focus entry, and Escape close behavior; `messk/src/lib/callControls.ts` keeps call control labels and live status roles explicit and free of raw signaling details. |
| Bundle and lazy-route boundaries | Implemented and regression-tested | `messk/src/components/AuthenticatedSession.tsx` isolates authenticated chat bootstrap from the public app shell; `scripts/frontend-bundle-budget.ps1` verifies emitted route chunks and blocks authenticated-only markers from the entry bundle. |
| Visual regression baselines | Implemented and regression-tested | `messk/visual-regression/scenarios.json` covers normal, empty, loading and recovered states across mobile, tablet and desktop; `scripts/visual-regression-baselines.ps1` verifies deterministic SVG baselines in `docs/visual-regression/`. |
...
powershell -ExecutionPolicy Bypass -File scripts\visual-regression-baselines.ps1
