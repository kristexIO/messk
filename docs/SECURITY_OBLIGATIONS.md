...
| Secret lifetime reduction | Windows X3DH and ratchet state use zeroization-on-drop, wipe decrypted persistence buffers, and clear seed UI copies; web vault, attachment, and encrypted-backup paths wipe mutable temporary key/plaintext buffers after use. Browser string/runtime copies remain a platform limitation. |
